...
% For loop to estimate circle constant
...
title('Estimation of Circle Constant vs Points Count');
legend('Circle Constant Estimate');
...
title('Deviation of Circle Constant Estimate');
drawnow;  % Force rendering
...
% Function to estimate the Circle Constant with a user-defined precision
...
function displayPoints(xCoord, yCoord, estValue, precisionLabel, formatSpec)
    ...
    title(sprintf('Monte Carlo - Circle Constant: %s (%s)', sprintf(formatSpec, estValue), precisionLabel), 'FontSize', 14);
    xlabel('X');
    ylabel('Y');
